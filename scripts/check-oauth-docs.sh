...
assert_contains site-docs/reference/cli-flags.md '`oauth_browser` is supported for HTTP only through `--mcp-config` and structured JSON/TOML `mcp_servers` configuration; compact `--mcp-server` syntax does not support it.'
