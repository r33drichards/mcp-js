...
fn test_rss_growth_stateless_no_wasm() {
    ...
    let (_before, _after, per_iter_kb) = measure_rss_growth(
        ...
        || {
            ...
            let _ = execute_stateless("1 + 1", heap_bytes, handle, &[], heap_bytes, None, None, None, None);
        },
    );
...
fn test_rss_growth_stateless_with_invalid_wasm() {
    ...
    let (_before, _after, per_iter_kb) = measure_rss_growth(
        ...
        || {
            ...
            let _ = execute_stateless("1", heap_bytes, handle, &modules, heap_bytes, None, None, None, None);
        },
    );
...
fn test_rss_growth_stateful_no_wasm() {
    ...
    let (_before, _after, per_iter_kb) = measure_rss_growth(
        ...
        || {
            let handle = no_handle();
            let _ = execute_stateful("1 + 1", None, heap_bytes, handle, &[], heap_bytes, None, None, None, None);
        },
    );
...
fn test_rss_growth_stateless_syntax_error() {
    ...
    let (_before, _after, per_iter_kb) = measure_rss_growth(
        ...
        || {
            let handle = no_handle();
            let _ = execute_stateless("= mon:= m {", heap_bytes, handle, &[], heap_bytes, None, None, None, None);
        },
    );
