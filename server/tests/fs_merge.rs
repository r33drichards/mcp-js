...
use server::engine::fs_merge::{merge_manifests, MergeResult, Prefer};
...
fn merged_or_panic(r: MergeResult) -> Manifest {
    assert!(
...
fn non_overlapping_changes_merge_cleanly() {
    ...
    let ours = manifest(&[("a", b"a1"), ("b", b"b0")]);     let theirs = manifest(&[("a", b"a0"), ("b", b"b1")]);     let m = merged_or_panic(merge_manifests(Some(&base), &ours, &theirs, Prefer::None));
...
fn one_sided_delete_is_applied() {
    let base = manifest(&[("a", b"a0"), ("b", b"b0")]);
    let ours = manifest(&[("a", b"a0")]);     let theirs = manifest(&[("a", b"a0"), ("b", b"b0")]);     let m = merged_or_panic(merge_manifests(Some(&base), &ours, &theirs, Prefer::None));
...
fn divergent_same_path_conflicts() {
    ...
    let r = merge_manifests(Some(&base), &ours, &theirs, Prefer::None);
    assert!(r.merged.entries.is_empty(), "nothing resolves cleanly");
    assert_eq!(r.conflicts.len(), 1);
    assert_eq!(r.conflicts[0].path, PathBuf::from("a"));
    assert!(
        r.conflicts[0].base.is_some()
            && r.conflicts[0].ours.is_some()
            && r.conflicts[0].theirs.is_some()
    );
}
...
fn modify_delete_is_a_conflict() {
    let base = manifest(&[("a", b"a0")]);
    let ours = manifest(&[("a", b"a1")]);     let theirs = manifest(&[]);     let r = merge_manifests(Some(&base), &ours, &theirs, Prefer::None);
    assert_eq!(r.conflicts.len(), 1);
    assert!(r.conflicts[0].ours.is_some());
    assert!(r.conflicts[0].theirs.is_none());
}
...
fn two_way_merge_without_base_conflicts_on_divergence_but_unions_disjoint() {
    ...
    let r = merge_manifests(None, &ours, &theirs, Prefer::None);
    assert_eq!(r.conflicts.len(), 1);
    assert_eq!(r.conflicts[0].path, PathBuf::from("a"));
    assert!(r.conflicts[0].base.is_none());
        assert!(r.merged.entries.contains_key(&PathBuf::from("only_ours")));
    assert!(r.merged.entries.contains_key(&PathBuf::from("only_theirs")));
