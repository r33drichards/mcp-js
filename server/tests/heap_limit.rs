...
fn test_stateless_small_heap_limit_rejects_large_allocation() {
    ...
    let result = server::mcp::execute_stateless(MEMORY_HOG_JS.to_string(), max_bytes);
...
fn test_stateless_default_limit_allows_small_code() {
    ...
    let result = server::mcp::execute_stateless(SMALL_JS.to_string(), max_bytes);
...
fn test_stateful_small_heap_limit_rejects_large_allocation() {
    ...
    // 5 MB heap limit — too small for a 2M-element string array
    let max_bytes = 5 * 1024 * 1024;
    let result = server::mcp::execute_stateful(MEMORY_HOG_JS.to_string(), None, max_bytes);
...
fn test_stateful_default_limit_allows_small_code() {
    ...
    let max_bytes = server::mcp::DEFAULT_HEAP_MEMORY_MAX_MB * 1024 * 1024;
    let result = server::mcp::execute_stateful(SMALL_JS.to_string(), None, max_bytes);
...
fn test_different_limits_produce_different_outcomes() {
    ...
    let small_result = server::mcp::execute_stateless(MEMORY_HOG_JS.to_string(), small_limit);
    let large_result = server::mcp::execute_stateless(MEMORY_HOG_JS.to_string(), large_limit);
