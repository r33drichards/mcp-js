...
fn test_stateless_small_heap_limit_rejects_large_allocation() {
    ...
    let (result, _oom) = server::engine::execute_stateless(MEMORY_HOG_JS, max_bytes, no_handle(), &[], max_bytes, None, None, None, None);
...
fn test_stateless_default_limit_allows_small_code() {
    ...
    let (result, _oom) = server::engine::execute_stateless(SMALL_JS, max_bytes, no_handle(), &[], max_bytes, None, None, None, None);
...
fn test_stateful_small_heap_limit_rejects_large_allocation() {
    ...
    // 5 MB heap limit — too small for a 2M-element string array
    let max_bytes = 5 * 1024 * 1024;
    let (result, _oom) = server::engine::execute_stateful(MEMORY_HOG_JS, None, max_bytes, no_handle(), &[], max_bytes, None, None, None, None);
...
fn test_stateful_default_limit_allows_small_code() {
    ...
    let max_bytes = 8 * 1024 * 1024;
    let (result, _oom) = server::engine::execute_stateful(SMALL_JS, None, max_bytes, no_handle(), &[], max_bytes, None, None, None, None);
...
fn test_different_limits_produce_different_outcomes() {
    ...
    let (small_result, _) = server::engine::execute_stateless(MEMORY_HOG_JS, small_limit, no_handle(), &[], small_limit, None, None, None, None);
    let (large_result, _) = server::engine::execute_stateless(MEMORY_HOG_JS, large_limit, no_handle(), &[], large_limit, None, None, None, None);
...
fn test_typed_array_oom_does_not_crash_stateless() {
    ...
    let (result, _oom) = server::engine::execute_stateless(TYPED_ARRAY_OOM_JS, heap_bytes, no_handle(), &[], heap_bytes, None, None, None, None);
...
fn test_typed_array_oom_does_not_crash_stateful() {
    ...
    let heap_bytes = 8 * 1024 * 1024;
    let (result, _oom) = server::engine::execute_stateful(TYPED_ARRAY_OOM_JS, None, heap_bytes, no_handle(), &[], heap_bytes, None, None, None, None);
...
fn extreme_oom_subprocess_worker() {
    ...
    match mode.as_str() {
        "stateless" => {
            let (result, _) = server::engine::execute_stateless(code, heap_bytes, no_handle(), &[], heap_bytes, None, None, None, None);
        ...
        "stateful" => {
            let (result, _) = server::engine::execute_stateful(code, None, heap_bytes, no_handle(), &[], heap_bytes, None, None, None, None);
            if result.is_err() {
                std::process::exit(0);
            }
            std::process::exit(0);
...
fn run_extreme_oom_subprocess(mode: &str) {
    ...
    ensure_v8();
    let (result, _) = server::engine::execute_stateless("1 + 1", 8 * 1024 * 1024, no_handle(), &[], 16 * 1024 * 1024, None, None, None, None);
    assert!(
        result.is_ok(),
