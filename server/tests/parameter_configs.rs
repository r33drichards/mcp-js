...
fn test_timeout_produces_descriptive_error() {
    ...
    let result = server::engine::execute_stateless(code, heap_bytes, timeout_secs);
...
fn test_timeout_stateful_produces_descriptive_error() {
    ...
    let result = server::engine::execute_stateful(code, None, heap_bytes, timeout_secs);
...
fn test_bare_call_default_params() {
    ...
    let timeout_secs = server::engine::DEFAULT_EXECUTION_TIMEOUT_SECS;
