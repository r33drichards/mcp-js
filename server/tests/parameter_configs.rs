...
fn test_oom_produces_descriptive_error_not_crash() {
    ...
    let (result, _oom) = server::engine::execute_stateless(code, heap_bytes, no_handle(), &[], heap_bytes, None, None, None, None);
...
fn test_oom_stateful_produces_descriptive_error_not_crash() {
    ...
    let (result, _oom) = server::engine::execute_stateful(code, None, heap_bytes, no_handle(), &[], heap_bytes, None, None, None, None);
...
fn test_bare_call_default_params() {
    ...
    let (result, _oom) = server::engine::execute_stateless("1 + 1", heap_bytes, no_handle(), &[], heap_bytes, None, None, None, None);
