//! `#[derive(UniffiBuilder)]` for `uniffi::Record` configuration types.
//!
//! For a record `Foo` annotated `#[uniffi_builder(ErrorType)]`, the derive
//! generates a `FooBuilder` `uniffi::Object` with a `new()` constructor, one
//! immutable setter per field (each returns a new `Arc<FooBuilder>`, so
//! foreign callers chain them), and `build() -> Result<Foo, ErrorType>`.
//! Fields typed `Option<T>` are optional; every other field must be set, and
//! `build` reports the first missing one through `ErrorType::missing(record,
//! field)`.
//!
//! Vendored from trycua/cua `libs/fleet/uniffi-builder-derive` (MIT, see
//! LICENSE in this directory) so mcp-js configuration records expose the same
//! builder shape across every generated language.
