...
builds it, installs `@earendil-works/pi-agent-core` from that checkout, and runs
...
npm install --prefix node/pi-harness
npm test --prefix node/pi-harness
```

`pi-checkout/` is ignored by git. The test is run with `tsx` and is not
