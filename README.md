...
- **OPA-Gated Subprocess Execution**: Optional `Deno.Command` and `child_process.exec` APIs for running OS commands, with every invocation checked against a Rego policy before execution.
- **MCP Pass-Through**: Connect to upstream MCP servers and call their tools from JavaScript via the `mcp` global (`mcp.callTool`, `mcp.listTools`, `mcp.servers`), gated by an optional MCP-tools policy.
- **Runtime Timers**: `setTimeout` and `clearTimeout` are always available (`setInterval` is not).
...
| `/api/cli` | GET | CLI binary download index |
| `/llms.txt` | GET | Machine-readable agent guide (llms.txt) |
| `/docs` | GET | Full README rendered as Markdown |
...
- **Limited timers**: `setTimeout` and `clearTimeout` are available (always enabled); `setInterval` / `clearInterval` are not.
