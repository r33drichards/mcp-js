...
Claude writes JavaScript that calls MCP tools as mcp.<server>.<tool>() inside the V8 sandbox.
...
PROMPT = (
    ...
    "Write JavaScript using the mcp.<server>.<tool>() API to query the GitHub MCP "
    "server connected as 'github'. The available JS API is:\n"
    "  mcp.servers            // string[] of connected server names\n"
    "  mcp.tools('github') // list available tools with schemas\n"
    "  await mcp.github.toolName({arg: 'val'}) // resolves to the unwrapped result value\n\n"
